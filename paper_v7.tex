...
In furthering causal set theory it is crucial that we understand the kinematics of the theory. One avenue in this direction is the attempt to define a discrete counterpart of the gravitational action $S[\mathcal C]$ associated with a causal set $\mathcal C$, which furnishes the weight $e^{iS[\mathcal C]}$ in the graviational sum over histories. A proposal for the action of a causal set have recently been made \cite{Benincasa_Dowker:The_Scalar_Curvature_of_a_Causal_Set}, and for a number of special cases, it has been shown that for causal sets that are well-approximated by continuum spacetimes (in a sense that will be made clear below), the proposed action agrees with the Einstein-Hilbert action of the spacetime in the appropriate continuum limit.\mb{given what conditions?}

In the continuum, it is well known that the Einstein-Hilbert action, $S_{EH}$, is not the full story. In the presence of spacetime boundaries the gravitational action must include a boundary term $S_{GHY}$, the Gibbons-Hawking-York action, in order to yield a well-defined variational principle~\cite{Gibbons_Hawking_Boundary}. The contributions of this term play an essential role in particular in the quantum theory. For instance, in the calculation of the black hole entropy via the Euclidean path-integral, it is the boundary terms that produce the answer $A/4l_p^2$ necessary for the unification of black hole mechanics with thermodynamics. To give another example, in Regge calculus boundary terms are necessary in order that the quantum mechanical amplitudes satisfy the correct composition law and that they have the correct classical limit~\cite{hartlesorkin}.\mb{is this true?}
...
In order to give intuition to the ideas laid out in this paper it will be most convenient to consider spacetimes with boundary obtained by partitioning a manifold (without boundary) into two parts as follows. Let $(M,g)$ be a sufficiently well-behaved $d$-dimensional Lorentzian manifold. We assume that $(M,g)$ has finite volume and admits a \emph{spacelike partitioning} surface, $\Sigma$, defined as a spacelike closed compact surface in $M$, such that the causal past and future sets, $M^\pm=J^\pm (\Sigma)$ satisfy $M^+\cap M^-=\Sigma$. It then follows that $\partial M^\pm = \Sigma$, and further that no points in $M^+\setminus\Sigma$ precede any points in $M^-\setminus\Sigma$.~\!\footnote{If there did exist $p\in M^+\setminus\Sigma$ and $q\in M^-\setminus\Sigma$ such that $p\preceq q$, then it would follow that $(q\in J^-(\Sigma) \text{ and } p\preceq q)\implies p\in J^-(\Sigma)\implies p\in M^-$, which contradicts $M^+\cap M^-=\Sigma$.} The Gibbons-Hawking-York boundary term for $M^\pm$ is then given by
...
Given a causet $\mathcal C$ that can be faithfully embedded in a manifold $M$ with a spacelike partitioning surface $\Sigma$, we obtain a natural partition of the causet into two parts $\mathcal{C}^+$ and $\mathcal{C}^-$, corresponding to the sets of elements that lie in $M^+=J^+(\Sigma)$ and $M^-=J^-(\Sigma)$. By virtue of $\Sigma$ being spacelike partitioning, the set $\mathcal C^-$ contains its own past, that is, any element in $\mathcal C$ that precedes at least one element in $\mathcal{C}^-$ is also in $\mathcal{C}^-$. A set with this property is known as a \emph{past stem}. Similarly, $\mathcal C^+$ is a \emph{future stem} given the analogous definition. We propose more generally that for any causet $\mathcal C$, a \emph{spacelike partition} is a non-trivial partition that splits $\mathcal{C}$ into two parts, $\mathcal{C}^-$ and $\mathcal{C}^+=\mathcal{C}\setminus\mathcal{C}^-$ \IGNORE{($\mathcal{C}=\mathcal{C}^-\cup\mathcal{C}^+$)}, such that $\mathcal{C}^-$ is a past stem (or, equivalently, that $\mathcal{C}^+$ is a future stem and $\mathcal{C}^- = \mathcal{C}\setminus\mathcal{C}^+$). A sprinkling into a manifold with a surface such as $\Sigma$ will of course produce a causal set with spacelike partition, but we shall refer to a partition of a causal set satisfying the above definition as spacelike even when the causet is not obtained via a sprinkling into some manifold.

We restrict our discussion to spacelike hypersurfaces, because the definition of the causet counterpart to the boundary action associated with a \emph{timelike} hypersurface is much less straight-forward, so much so that no satisfactory candidate has yet been found. The special case for null hypersurfaces deserves some comments. First, it appears that while the GHY boundary term is well defined for space-like and time-like boundaries in the continuum, it is an open question what the contribution from a null boundary is~\cite{Gibbons_Hawking_Boundary, Harris}.\mb{copied this from sumati's section before seeing her newest edits, so beware of repetition.} Second, in the discrete setting, the distinction between a null and a spacelike hypersurface loses its meaning to some extent, since any null hypersurface is ``infinitesimally close'' to a spacelike hypersurface. Hence a finite causet cannot distinguish a null hypersurface from a spacelike hypersurface. To see why this is, imagine a causal set obtained via sprinkling into a patch of $2$-dimensional Minkowski space. Figure~\ref{fig:low_density_sprinkling_with_null_and_spacelike} shows how, given a null surface that partitions the causet into two parts, we can tilt the surface ever so slightly to make it spacelike, while still preserving the partition. Figure~\ref{fig:high_density_sprinkling_with_null_and_spacelike} shows that even as we increase the density of the sprinkling we can tilt the surface by a smaller amount such that the partition remains unchanged. A finite causet faithfully embeddable into a manifold with a null hypersurface therefore has no way of knowing whether the hypersurface is null or spacelike. With these comments in mind, we return to the spacelike case but we shall return to the question of null boundaries below.
...
The integrals~\eqref{eq:nmax} and~\eqref{eq:nmin} seem intractable as they stand, since the integration is over the entire causal past/future of the surface and the volume expressions will be complicated in the presence of curvature. However, due to $\Sigma$ being closed compact and $M^+$ and $M^-$ of finite volume, we can always find a subneighbourhood $U^{\varepsilon_{}}_\Sigma$ of $U_\Sigma$ such that the contribution to the boundary term from points outside of that neighbourhood tends to zero exponentially quickly as $\rho\rightarrow\infty$.\footnote{To see this, let us restrict ourselves to points to the future of $\Sigma$ and hence to the contributions to $\mathbf F_k$. In the GNC system, there will be some $t_{max}>0$ such that all points $(t,\mathbf x)$ with $|t|<t_{max}$ lie in $U_\Sigma$. Choose some $\epsilon<t_{max}$, and consider the subneighbourhood $U^{\varepsilon_{}}_\Sigma=\left\{(t,\mathbf x)\in U_\Sigma: |t|<\epsilon\right\}$. Then given any $\Lambda\in\mathbb R^+$, there exists, for each $\mathbf x$, a value $\rho_\Lambda(\mathbf x)$ such that $V_\blacktriangle(\mathbf x, t)\rho_\Lambda(\mathbf x)>\Lambda$ for all points outside of $U^{\varepsilon_{}}_\Sigma$. Since $\Sigma$ is compact, $\rho_\Lambda(\mathbf x)$ achieves its maximum on $\Sigma$ and we can choose a finite constant value $\rho_\Lambda\equiv\max_{\mathbf{x}}\rho_\Lambda(\mathbf x)$ such that $\rho_\Lambda V(\mathbf x, t)>\Lambda$ for all $x\in M^+\setminus U^{\varepsilon_{}}_\Sigma$. Since $M^+$ has finite volume, the total contribution from the points outside $U^{\varepsilon_{}}_\Sigma$ to $\left<\mathbf P_k\right>$ is then bounded from above by $\rho_\Lambda V(M^+\setminus U^{\varepsilon_{}}_\Sigma)\frac{\Lambda^k}{k!}e^{-\Lambda}$ for large enough $\Lambda$, a bound that can be made arbitrarily small.} 
...
We need to assume that for any $p$ in $U^{\varepsilon_{}}_\Sigma$, the Riemann normal neighbourhood throughout which the RNC system centered at $p_0$ is well-defined contains the truncated cone $J^- (p)\cap J^+ (\Sigma)$. This assumption seems reasonable given the that $U^{\varepsilon_{}}_\Sigma$ can be made arbitrarily ``thin'' as $\rho\rightarrow\infty$. In RNCs the metric and the Christoffel symbols at $p_0$ are those of flat space:
...
\begin{figure}[t]
  \centering
    {\includegraphics[scale=0.5]{coneplot2}}
     \caption{A $3$-dimensional representation of the region $\mathcal{X}_p$ in RNCs. The top part of $\partial\mathcal{X}_p$ can be approximated as a flat cone, and the base surface intersects the top part at a radial coordinate, $r_{max} (\boldsymbol\phi)$, which will in general be a function of the angles $\boldsymbol\phi$ (in three dimensions there is one angle $\phi$). This function is found by projecting down from the intersection to the $\overline{t}$ plane. The size of the region inside the top and bottom bounding surfaces is the volume we want to calculate.}
     \label{fig:cone_plot}
\end{figure}
