...
\ee
where $A^{\mu}_{\;\overline{\mu}}$ is the inverse matrix of $A^{\overline\mu}_{\;{\mu}}$, i.e. $A^{\overline{\mu}}_{\;\mu}A^{\mu}_{\;\overline{\nu}}=\delta^{\overline{\mu}}_{\;\overline{\nu}}$ and $A^{\mu}_{\;\overline{\mu}}A^{\overline{\mu}}_{\;\nu}=\delta^{\mu}_{\;\nu}$. There is no $O(y^2)$ term in~\eqref{eq:RNCinversetrans} due to the fact that $\Gamma^{\overline{\mu}}_{\;\overline{\nu}\overline{\rho}} (p_0)=0$. Let us denote the time-coordinate of $p$ in GNCs by $T=x^0_p=t_p$. From \cite{Khetrapal_Sumati:Causal_Diamond_Volume} we have that the tip of the cone in RNC's is $y^{\overline{0}}_p=\overline{t}_p=T+O(T^3)$, which, along with~\eqref{eq:RNCMetricTransAtPAndChris}, sets the components of $A^{\overline{\mu}}_{\mu}$ to be $A^{\overline 0}_{\;0}=1$, $A^{\overline 0}_{\;i}=0$ and $A^{\ibar}_{\;i}A^{\jbar}_{\;j}\delta_{ij} =h_{ij}(p_0)\iff\delta_{\ibar\jbar}=A^i_{\;\ibar}A^j_{\;\jbar}h_{ij} (p_0)$.
