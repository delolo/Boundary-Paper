...
\ee
Where we can see that when $x^{\mu}=x_0^{\mu}$ we have that $y^{\overline{\mu}}=0$, and hence that our coordinates are centred at $p_0$. The inverse relation to first order is
\be\label{eq:RNCinversetrans}
x^{\mu}=x_0^{\mu}+A^{\mu}_{\;\overline{\nu}}y^{\overline{\nu}}+O (y^3)
\ee
where the $O(y^2)$ term vanishes due to the fact that $\Gamma^{\overline{\mu}}_{\;\overline{\nu}\overline{\rho}} (p_0)=0$. One finds that the $A^{\mu}_{\;\overline{\mu}}$ must satisfy
...
\ee
Let us denote the time-coordinate of $p$ in GNCs by $T=x^0_p=t_p$. For the transformation from GNCs to RNCs at $p_0$ one finds that $A^{\overline 0}_{\;0}=1$, $A^{\overline 0}_{\;i}=0$ and $\delta_{\ibar\jbar}=A^i_{\;\ibar}A^j_{\;\jbar}h_{ij} (p_0)$, which in particular implies $y^{\overline{0}}_p=\overline{t}_p=T+O(T^3)$.
...
\ee
The linear part on the right of (\ref{eq:RNCtotaltrans}) vanishes as $A^{\overline{0}}_{\;\mu}(x^{\mu}-x_0^{\mu})=t$ (as $A^{\overline{0}}_{\;i}=0$ and $A^{\overline{0}}_{\;0}=1$) and $t=0$ for the bottom surface. Using the inverse RNC relation (\ref{eq:RNCinversetrans}) one can find the equation for the bottom surface in RNCs:
