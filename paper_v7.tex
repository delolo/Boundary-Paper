...
\ee
Where we can see that when $x^{\mu}=x_0^{\mu}$ we have that $y^{\overline{\mu}}=0$, and hence that our coordinates are centred at $p_0$. The inverse coordinate transformation to first order is
...
\ee
Let us denote the time-coordinate of $p$ in GNCs by $T=x^0_p=t_p$. Equation~\eqref{eq:RNCMetricTransAtPAndChris} is satisfied if we set $A^{\overline 0}_{\;0}=1$, $A^{\overline 0}_{\;i}=0$ and $\delta_{\ibar\jbar}=A^i_{\;\ibar}A^j_{\;\jbar}h_{ij} (p_0)$. These relations, along with~\eqref{eq:RNCtotaltrans}, imply that $y^{\overline{0}}_p=\overline{t}_p=T+O(T^3)$.
