...
\usepackage{amsmath}
\usepackage{jheppub}
\usepackage{marginnote,xparse,changepage,caption,subfigure}
...
Consider a sufficiently well-behaved $d$-dimensional spacetime $ (M,g)$ and a surface $\Sigma$ in $M$, which for now we take to be a Cauchy surface. The causal past and future sets $M^\pm=J^\pm (\Sigma)$ of the surface then form a partition of $M$, and $\partial M^\pm = \Sigma$. The Gibbons-Hawking-York boundary term for $M^\pm$ is in this case given by
\be\label{eq:GHYBT_in_continuum}
\mathcal{S}_{GHY} = \mp \frac{1}{l_p^{d-2}}\int_{\Sigma} K d\Sigma
\ee
...
Spacetime volume in the causal set is obtained simply by counting the number of elements, and hence the volume gradient intuitively corresponds to the difference in the number of ``immediate neighbours'' to the future and to the past of a hypersurface. What does it mean to be immediate neighbours to a surface in a causal set? In order to make this notion precise, we need to define the causal set analogue of a hypersurface, as well as an appropriate notion of ``closeness''. This turns out to be most natural for the analogue of what in the continuum would correspond to a Cauchy hypersurface. 

In the causet, a (partitioning) hypersurface can be described as a particular partition of the causet. \mb{why say this here?}We will only use this definition for causets of finite cardinality.Define a \emph{past} \emph{stem} to be a subset $\mathcal{C}'\subseteq\mathcal{C}$ of the causet that contains its own past\IGNORE{, i.e. $(\exists x\in\mathcal{C}' \text{ such that } y\prec x)\implies y\in\mathcal{C}'$}. That is, any element in the causet that precedes at least one element in $\mathcal{C}'$ is also in $\mathcal{C}'$. A \emph{future stem} is defined analogously. Let us now define a \emph{spacelike partition} as one which partitions the causet  $\mathcal{C}$ into two parts, $\mathcal{C}^-$ and $\mathcal{C}^+$\IGNORE{($\mathcal{C}=\mathcal{C}^-\cup\mathcal{C}^+$)}, where $\mathcal{C}^-$ is a past stem and $\mathcal{C}^+$ is its complement, $\mathcal{C}^+=\mathcal{C}\setminus\mathcal{C}^-$. One can equivalently pick some future stem $\mathcal{C}^+$ and its complement $\mathcal{C}^- = \mathcal{C}\setminus\mathcal{C}^+$ to define a spacelike partition. The reason that we call some partition of this nature ``spacelike" is the following. Given a causet that can be faithfully embedded in some manifold, $M$, then given a spacelike hypersurface, $\Sigma$, we can partition the causet into two bits\mb{only true if $\Sigma$ is Cauchy, no?}, denoting the future (past) part as $\mathcal{C}^+$ ($\mathcal{C}^-$), which is made up of those elements that lie in $M^+$ ($M^-$). The causal relations between the two parts, that can be found from the causal strucutre of the manifold, immediately satisfy our requirement for the partition to be spacelike. We will still refer to a partition as \emph{spacelike} even if the causet is not faithfully embeddable in some manifold. For immediate neighbours to the past of the partition we mean elements of $\mathcal{C}^-$ that have no elements in their causal future in $\mathcal{C}^-$ (these elements being called \textit{maximal} elements in $\mathcal{C}^-$). We assume an analogous definition for immediate neighbours to the $future$ of the partition, which we shall refer to as minimal elements in $\mathcal{C}^-$. We shall see that the intuitive idea for the boundary term as the ``rate of change of spacetime atoms'' across a partition indeed bears out. However, one does not need to know the maximal and minimal elements to calculate the boundary term. It can be found from any elements that lie reasonably close to the surface (in the time coordinate on $M$) but are not necessarily maximal or minimal. This will be made more precise below.
...
\IGNORE{
        ...
        \caption{The partition induced by the sprinkling is unchanged after tilting the surface between null and spacelike. With an even higher density, as in (b), one can simply titl the surface by some smaller amount to keep the partition unaltered.}\label{fig:animals}
\end{figure}

Let us define the functions $N_{\vphantom{i}max}^{ (k)}\left[\mathcal{C} \right]$ and $N_{min}^{ (k)}\left[\mathcal{C}^- \right]$ that, for a given finite causet, $\mathcal{C}$, return the number of $k$\emph{-next-to-maximal}/\emph{minimal} elements respectively. A $k$-next-to-maximal (minimal) element is one for which there are $k$ elements to its future (past). This means that the $0$-next-to-maximal (minimal) elements are the maximal (minimal) elements.


Given a causet, $\mathcal{C}$, with a spacelike partition so that $\mathcal C = \mathcal C^+ \cup\, \mathcal C^-$, we propose the following family of functions for calculating the GHY boundary term of this partition:
\be\label{general_boundary_sum}
S^{ (d)}_{GHY}\left[\mathcal{C}^-,\mathcal{C}^+;\vec{p}, \vec{q} \right]= \left (l/l_p\right)^{d-2} c_{d}
\left ( \sum_m p_m N_{\vphantom{i}max}^{ (m)}\left[\mathcal{C}^- \right]
...
\ee
and where $V_d=\pi^{\frac{d}{2}}/\Gamma\left (\frac{d}{2}+1\right)$ denotes the volume of the unit $d$-ball ($\Gamma (t)$ is a gamma function, defined as $\Gamma (t):=\int_0^\infty dx\: x^{t-1}\: e^{-x}$). We have used $\vec{p}$ and $\vec{q}$ to stand for infinite dimensional vectors $ (p_0,...,p_m,..)$ and $ (q_0,...,q_n,..)$ respectively, where all the $p_m,\: q_n \in \mathbb{R}$. These vectors contain a finite number of non-zero entries, corresponding to the coefficients of the different $N_{{max\vphantom{i}}}^{\: (m)}$ and $N_{{min}}^{\: (n)}$ terms. We use the summation $\sum_m$ ($\sum_n$) to denote a sum that runs from $0$ to $m_{max}$ ($n_{max}$), where $m_{max}$ ($n_{max}$) is some integer, the value of which depends on the causet $\mathcal{C}^-$ ($\mathcal{C}^+$), for which all $N_{\vphantom{i}max}^{ (m)}\left[\mathcal{C}^- \right]=0$ ($N_{min}^{ (n)}\left[\mathcal{C}^+ \right]=0$) for any $m>m_{max}$ ($n>n_{max}$). A finite value for $m_{max}$ ($n_{max}$) will always exist for any finite causet, as it will not be large enough or be structured in such a way to have any $m$-next-to-maximal ($n$-next-to-minimal) elements for $m>m_{max}$ ($n>n_{max}$). The vectors must be chosen such that they satisfy the following conditions.
